...
//
// Table class for tools_backups
//
class ctools_backups extends cTable {
	var $idtools_backups;
	var $domain_id;
	var $data;
	var $date;
	...
	function __construct() {
		...
		$this->TableVar = 'tools_backups';
		$this->TableName = 'tools_backups';
		...
		// idtools_backups
		$this->idtools_backups = new cField('tools_backups', 'tools_backups', 'x_idtools_backups', 'idtools_backups', '`idtools_backups`', '`idtools_backups`', 3, -1, FALSE, '`idtools_backups`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->idtools_backups->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['idtools_backups'] = &$this->idtools_backups;

		// domain_id
		$this->domain_id = new cField('tools_backups', 'tools_backups', 'x_domain_id', 'domain_id', '`domain_id`', '`domain_id`', 200, -1, FALSE, '`domain_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['domain_id'] = &$this->domain_id;

		// data
		$this->data = new cField('tools_backups', 'tools_backups', 'x_data', 'data', '`data`', '`data`', 205, -1, TRUE, '`data`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['data'] = &$this->data;

		// date
		$this->date = new cField('tools_backups', 'tools_backups', 'x_date', 'date', '`date`', 'DATE_FORMAT(`date`, \'%Y/%m/%d %H:%i:%s\')', 135, 5, FALSE, '`date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailFilter() {
		...
		if ($this->getCurrentMasterTable() == "domains") {
			if ($this->domain_id->getSessionValue() <> "")
				$sDetailFilter .= "`domain_id`=" . ew_QuotedValue($this->domain_id->getSessionValue(), EW_DATATYPE_STRING);
			else
				return "";
		}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('idtools_backups') . '=' . ew_QuotedValue($rs['idtools_backups'], $this->idtools_backups->FldDataType) . ' AND ';
		}
	...
	function SqlKeyFilter() {
		return "`idtools_backups` = @idtools_backups@";
	}

	// Key filter
	function KeyFilter() {
		$sKeyFilter = $this->SqlKeyFilter();
		if (!is_numeric($this->idtools_backups->CurrentValue))
			$sKeyFilter = "0=1"; // Invalid key
		$sKeyFilter = str_replace("@idtools_backups@", ew_AdjustSql($this->idtools_backups->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return $this->james_url( "tools_backupslist.php" );
		}
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl($this->james_url("tools_backupsview.php"), $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return $this->james_url("tools_backupsadd.php");
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl($this->james_url("tools_backupsedit.php"), $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl($this->james_url("tools_backupsadd.php"), $this->UrlParm($parm));
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl($this->james_url("tools_backupsdelete.php"), $this->UrlParm());
	}
	...
	function KeyUrl($url, $parm = "") {
		$sUrl = $url . "?";
		if ($parm <> "") $sUrl .= $parm . "&";
		if (!is_null($this->idtools_backups->CurrentValue)) {
			$sUrl .= "idtools_backups=" . urlencode($this->idtools_backups->CurrentValue);
		} else {
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["idtools_backups"]; // idtools_backups
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->idtools_backups->CurrentValue = $key;
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				if ($this->idtools_backups->Exportable) $Doc->ExportCaption($this->idtools_backups);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					if ($this->idtools_backups->Exportable) $Doc->ExportField($this->idtools_backups);
