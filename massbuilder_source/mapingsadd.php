...
$mapings_add = NULL; // Initialize page object first

class cmapings_add extends cmapings {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["idmapings"] != "") {
				...
				$this->setKey("idmapings", $this->idmapings->CurrentValue); // Set up key
			} else {
				$this->setKey("idmapings", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "mapingsview.php")
	...
	function LoadDefaultValues() {
		...
		$this->kind->CurrentValue = NULL;
		$this->kind->OldValue = $this->kind->CurrentValue;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// kind
		$this->kind->SetDbValueDef($rsnew, $this->kind->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$this->idmapings->setDbValue($conn->Insert_ID());
			$rsnew['idmapings'] = $this->idmapings->DbValue;
		}
		if ($AddRow) {
...
// Create page object
if (!isset($mapings_add)) $mapings_add = new cmapings_add();

// Page init
$mapings_add->Page_Init();

// Page main
$mapings_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var mapings_add = new ew_Page("mapings_add");
mapings_add.PageID = "add"; // Page ID
var EW_PAGE_ID = mapings_add.PageID; // For backward compatibility

// Form object
var fmapingsadd = new ew_Form("fmapingsadd");

// Validate form
fmapingsadd.Validate = function(fobj) {
...
fmapingsadd.Form_CustomValidate =  
...
fmapingsadd.ValidateRequired = true;
<?php } else { ?>
fmapingsadd.ValidateRequired = false; 
<?php } ?>
...
<h4 id="ewPageCaption" class="page-header ewTitle ewTableTitle"> <?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $mapings->TableCaption() ?></h4>
...
<?php $mapings_add->ShowPageHeader(); ?>
<?php
$mapings_add->ShowMessage();
?>
<form name="fmapingsadd" id="fmapingsadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<input type="hidden" name="t" value="mapings" />
<input type="hidden" name="a_add" id="a_add" value="A" />
<table id="tbl_mapingsadd" class="ewTable ewTableSeparate table table-striped ">
