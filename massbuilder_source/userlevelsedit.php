...
class cuserlevels_edit extends cuserlevels {
	...
	function Page_Main() {
		...
		if ($this->userlevelid->CurrentValue == "")
			$this->Page_Terminate($this->james_url( "userlevelslist.php" )); // Invalid key, return to list
		...
		switch ($this->CurrentAction) {
			case "I": // Get a record to display
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate($this->james_url( "userlevelslist.php" )); // No matching record, return to list
				}
				break;
			Case "U": // Update
...
<h4 id="ewPageCaption" class="page-header ewTitle ewTableTitle"> <?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $userlevels->TableCaption() ?></h4>
<a href="<?php echo $userlevels->getReturnUrl() ?>" id="a_GoBack" class="ewLink label"><i class="icon-arrow-left icon-white"></i> <?php echo $Language->Phrase("GoBack") ?></a>
...
<table id="tbl_userlevelsedit" class="ewTable ewTableSeparate table table-striped ">
<?php if ($userlevels->userlevelid->Visible) { // userlevelid ?>
	<tr id="r_userlevelid"<?php echo $userlevels->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_userlevels_userlevelid">
		<b><?php echo $userlevels->userlevelid->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></b>
		</span></td>
...
<?php if ($userlevels->userlevelname->Visible) { // userlevelname ?>
	<tr id="r_userlevelname"<?php echo $userlevels->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_userlevels_userlevelname">
		<b><?php echo $userlevels->userlevelname->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></b>
		</span></td>
