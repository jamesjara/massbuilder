...
//    }
    ...
    $family = mb_strtolower(DOMPDF_DEFAULT_FONT); //***

    if (isset(self::$_font_lookup[$family][$subtype])) {
      return self::$_font_lookup[$family][$subtype];
    } else { //***
      return DOMPDF_DEFAULT_FONT; //***
    } //***
...
//  }
  static function load_font_families() { //***
	...
	if (strtoupper(DOMPDF_PDF_BACKEND) == "CPDF") { // *** load CPDF fonts
	    ...
	    if ( self::$_font_lookup === 1 ) {
	      ...
	      self::$_font_lookup = require_once(self::CACHE_FILE);
    ...
    } elseif (strtoupper(DOMPDF_PDF_BACKEND) == "TCPDF") { // *** load TCPDF fonts
			$fontdir = DOMPDF_LIB_DIR . '/tcpdf/fonts/';
			$arfont = array('courier', 'helvetica', 'times',
				'almohanad', 'dejavusans', 'dejavusanscondensed',
				'dejavusansmono', 'dejavuserif', 'dejavuserifcondensed', 'freesans', 'freeserif', 'freemono');
			foreach ($arfont as $fontname) {
				if (file_exists($fontdir . $fontname . ".php")) {
					self::$_font_lookup[$fontname] = array(
			...
			if (file_exists($fontdir . "arialunicid0.php")) {
				self::$_font_lookup['arialunicodems'] = array(
						'normal' => $fontdir . 'arialunicid0',
				    'bold' => $fontdir . 'arialunicid0',
				    'italic' => $fontdir . 'arialunicid0',
				    'bold_italic' => $fontdir . 'arialunicid0');
			}
		}
  }
