...
$this->_page_text = array ();
	...
	protected function _place_objects() {
		...
		foreach ($this->_objs as $obj=>$props) {
			...
			($where == "all"))) {
				...
				//dompdf_debug("trace", "object data = " . $data['c']);
				$this->_pdf->setPageBuffer($page_number, $data['c'], true);
			}
		}
	...
	protected function _set_stroke_color($color) {
		//dompdf_debug("trace", "([$color[0], $color[1], $color[2]])");
	...
	*/
	...
	protected function _set_line_transparency($mode, $opacity) {
		//dompdf_debug("trace", "($mode, $opacity)");
	...
	protected function _set_line_style($width, $cap, $join, $dash) {
		//dompdf_debug("trace", "($width, $cap, $join, $dash)");
		//var_dump($dash);
		if ($dash) {
			$dash = implode(',', $dash);
		}
	...
	*/
	...
	protected function _make_line_style($color = '', $width='', $cap='', $join='', $dash='') {
		//dompdf_debug("trace", "($color, $width, $cap, $join, $dash)");
		$style = array();
		if ($color) {
	...
	protected function _convert_gif_to_png($image_url) {
		...
		if ($tmp_img[2] == '1') {
			...
			if ($img) {
				...
				$img_url = DOMPDF_LIB_DIR."/res/broken_image.png";
			}
		}
...
//			$s .= 'I';
//		}
		//dompdf_debug("trace2", "($font): returns $f, $s");
	 ...
	 */
	function get_width() {
		//dompdf_debug("trace", ": returns $this->_width");
		return $this->_width;
	}
	 ...
	 */
	function get_height() {
		//dompdf_debug("trace", ": returns $this->_height");
		return $this->_height;
	}
	...
	function rectangle($x1, $y1, $w, $h, $color, $width, $style = null, $blend = "Normal", $opacity = 1.0) {
		//dompdf_debug("trace", "($x1, $y1, $w, $h, [$color[0], $color[1], $color[2]], $width, $style, $blend, $opacity)");
  ...
  function transform($a, $b, $c, $d, $e, $f) {}
	...
	function polygon($points, $color, $width = null, $style = null, $fill = false, $blend = "Normal", $opacity = 1.0) {
		//dompdf_debug("trace", "($points, [$color[0], $color[1], $color[2]], $width, $style, $fill, $blend, $opacity)");
	...
	function circle($x, $y, $r, $color, $width = null, $style = null, $fill = false, $blend = "Normal", $opacity = 1.0) {
		//dompdf_debug("trace", "($x, $y, $r, [$color[0], $color[1], $color[2]], $width, $style, $fill, $blend, $opacity)");
...
//		}
	...
	function add_link($url, $x, $y, $width, $height) {
		//dompdf_debug("trace", "($url, $x, $y, $width, $height)");
	...
	function get_text_width($text, $font, $size, $word_spacing = 0, $char_spacing = 0) {
		...
		//dompdf_debug("trace", "($text, ". basename($font) .", $size, $word_spacing, $char_spacing): returns $result");
		return $result;
	}
	...
	function get_font_height($font, $size) {
		...
		$result = $scale * $size;
		//dompdf_debug("trace", "(". basename($font) .", $size): returns $result");
		return $result;
	}
	...
	function set_opacity($opacity, $mode = "Normal") {
		//dompdf_debug("trace", "($opacity, $mode)");
	...
	function stream($filename, $options = null) {
		//dompdf_debug("trace", "($filename, $options)");
	...
	function page_text($x, $y, $text, $font, $size, $color = array(0,0,0), $adjust = 0, $angle = 0) {
		//dompdf_debug("trace", "($x, $y, $text, ". basename($font) .", $size, $color, $adjust, $angle, $blend, $opacity)");
...
class My_TCPDF extends TCPDF {
	 ...
	 */
	function openObject() {
		//dompdf_debug("trace", "Enter My_TCPDF.openObject(state = $this->state)");
		...
		//dompdf_debug("trace", "---------> " . $curr_buffer);
	...
	function closeObject() {
		//dompdf_debug("trace", "Enter My_TCPDF.closeObject(state = $this->state)");
		if ($this->dompdf_num_stack > 0) {
			...
			//dompdf_debug("trace", "--------- OBJECT buffer -------> " . $curr_buffer);

			$saved_stack = $this->dompdf_stack[$this->dompdf_num_stack];

			//dompdf_debug("trace", "--------- saved buffer -------> " . $saved_stack['c']);
	...
	function reopenObject($id) {
		//dompdf_debug("trace", "Enter My_TCPDF.reopenObject($id)");
