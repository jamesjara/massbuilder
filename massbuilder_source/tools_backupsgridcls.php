...
$tools_backups_grid = NULL; // Initialize page object first

class ctools_backups_grid extends ctools_backups {
	...
	// Page object name
	var $PageObjName = 'tools_backups_grid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["tools_backups"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->idtools_backups->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_domain_id") && $objForm->HasValue("o_domain_id") && $this->domain_id->CurrentValue <> $this->domain_id->OldValue)
			return FALSE;
		if (!ew_Empty($this->data->Upload->Value))
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $this->idtools_backups->CurrentValue . "\" />";
		}
	...
	function GetUploadFiles() {
		...
		$this->data->Upload->RestoreDbFromSession();
		if ($confirmPage) { // Post from confirm page
			$this->data->Upload->RestoreFromSession();
		} else {
			if ($this->data->Upload->UploadFile()) {

				// No action required
			} else {
				echo $this->data->Upload->Message;
				$this->Page_Terminate();
				exit();
			}
			$this->data->Upload->SaveToSession();
			$this->data->CurrentValue = $this->data->Upload->FileName;
		}
	}

	// Load default values
	function LoadDefaultValues() {
		$this->idtools_backups->CurrentValue = NULL;
		$this->idtools_backups->OldValue = $this->idtools_backups->CurrentValue;
		...
		$this->data->Upload->Index = $this->RowIndex;
		$this->data->Upload->RemoveDbFromSession();
	...
	function LoadFormValues() {
		...
		if (!$this->idtools_backups->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idtools_backups->setFormValue($objForm->GetValue("x_idtools_backups"));
	...
	function RestoreFormValues() {
		...
		if ($this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
			$this->idtools_backups->CurrentValue = $this->idtools_backups->FormValue;
	...
	function LoadRowValues(&$rs) {
		...
		$this->data->Upload->Index = $this->RowIndex;
		$this->data->Upload->SaveDbToSession();
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->idtools_backups->CurrentValue = strval($arKeys[0]); // idtools_backups
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				$rswrk = $conn->Execute($sSqlWrk);
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					$this->domain_id->EditValue = $rswrk->fields('DispFld');
					$rswrk->Close();
				} else {
					$this->domain_id->EditValue = $this->domain_id->CurrentValue;
				}
			} else {
				$this->domain_id->EditValue = NULL;
			}
			}
			...
			if (!ew_Empty($this->data->Upload->DbValue)) {
				$this->data->EditValue = $this->data->FldCaption();
			} else {
				$this->data->EditValue = "";
			}
			...
			// idtools_backups
			$this->idtools_backups->EditCustomAttributes = "";
			$this->idtools_backups->EditValue = $this->idtools_backups->CurrentValue;
			$this->idtools_backups->ViewCustomAttributes = "";
	...
	function ValidateForm() {
		...
		if (!ew_CheckFileType($this->data->Upload->FileName)) {
			ew_AddMessage($gsFormError, $Language->Phrase("WrongFileType"));
		}
		if ($this->data->Upload->FileSize > 0 && EW_MAX_FILE_SIZE > 0 && $this->data->Upload->FileSize > EW_MAX_FILE_SIZE) {
			ew_AddMessage($gsFormError, str_replace("%s", EW_MAX_FILE_SIZE, $Language->Phrase("MaxFileSize")));
		}
		if (in_array($this->data->Upload->Error, array(1, 2, 3, 6, 7, 8))) {
			ew_AddMessage($gsFormError, $Language->Phrase("PhpUploadErr" . $this->data->Upload->Error));
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			if ($this->data->Upload->Action == "1") { // Keep
			} elseif ($this->data->Upload->Action == "2" || $this->data->Upload->Action == "3") { // Update/Remove
			if (is_null($this->data->Upload->Value)) {
				$rsnew['data'] = NULL;
			} else {
				$rsnew['data'] = $this->data->Upload->Value;
			}
			$this->data->SetDbValueDef($rsnew, $this->data->Upload->FileName, NULL, FALSE);
			}
			}
		...
		// data
		$this->data->Upload->RemoveFromSession(); // Remove file value from Session
		return $EditRow;
	}
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$this->idtools_backups->setDbValue($conn->Insert_ID());
			$rsnew['idtools_backups'] = $this->idtools_backups->DbValue;
		}
		if ($AddRow) {
