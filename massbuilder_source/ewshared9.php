...
//jamesjara changes
	...
	//			$str .= " class='dropdown' id=\"menu_" . $this->Id . "\"";
			} else {
	...
	//	$str .= "<div  class=\"bd" . (($this->IsRoot) ? " first-of-type": "") . "\">\n";
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				//jamesjar boostrap menu submenu
				if ($item->ParentId > 0 ) {
					$classfot = " class=\"dropdown-menu\"";
				}
				...
				if ($item->GroupTitle && EW_MENU_ITEM_CLASSNAME <> "") { // Group title
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							if ($this->RenderItem($subitem))
								...
								//jamesjara agrego bootstrap values
								$str .= $subitem->Render('dropdown-toggle', 'dropdown', false , 'dropdown' ) . "\n"; // Create <LI>
						}
					}
					$str .= "</ul>\n"; // End the group
					$gopen = FALSE;
					...
					$str .= $item->Render($aclass, $liclass, $this->IsMobile , $dropdown  ) . "\n"; // Create <LI>
				}
			}
		}
		if ($gopen)
...
class cMenuItem {
	...
	// Render
	function Render($aclass = "", $liclass = "", $mobile = FALSE, $datatoggle = null , $kind_element = "li") { //jamesjara agregue $datatoggle,kind_element
 $more = '';
		...
		if ($mobile)
			...
			//jamesjara $attrs = array("class" => $aclass, "href" => $this->Url, "target" => $this->Target);
			 $attrs = array("class" => $aclass,"data-toggle" => $datatoggle, "href" =>  $this->james_url($this->Url), "target" => $this->Target);	
		if (!is_null($this->SubMenu)) $more = '<b class="caret"></b>';
		$innerhtml = ew_HtmlElement("a", $attrs, $this->Text. $more);
		if (!is_null($this->SubMenu)) {
			if ($mobile && $this->Url <> "")
				$innerhtml .= $innerhtml;
		...
		// Create <LI>
		return ew_HtmlElement( $kind_element  , array("class" => $liclass), $innerhtml);
	}
}
...
// $UserAgent[0] = OS, [1] = agent, [2] = mobile, [3] = major version, [4,...] = sub versions if exist
function ew_IsMobile() {
	global $UserAgent, $EW_USE_MOBILE_MENU;
	if (!isset($UserAgent))
		$UserAgent = ew_UserAgent();
	return false; //jamesjara $EW_USE_MOBILE_MENU && $UserAgent[2] <> "";
}
?>
