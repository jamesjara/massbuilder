...
$entries_grid = NULL; // Initialize page object first

class centries_grid extends centries {
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["entries"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->identries->CurrentValue;
					if ($sKey <> "") $sKey .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_id") && $objForm->HasValue("o_id") && $this->id->CurrentValue <> $this->id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_islive") && $objForm->HasValue("o_islive") && $this->islive->CurrentValue <> $this->islive->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_tool_id") && $objForm->HasValue("o_tool_id") && $this->tool_id->CurrentValue <> $this->tool_id->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $this->identries->CurrentValue . $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"] . $this->id->CurrentValue . "\" />";
		}
	...
	function LoadFormValues() {
		...
		$this->islive->setOldValue($objForm->GetValue("o_islive"));
		if (!$this->tool_id->FldIsDetailKey) {
			$this->tool_id->setFormValue($objForm->GetValue("x_tool_id"));
		}
		$this->tool_id->setOldValue($objForm->GetValue("o_tool_id"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->identries->CurrentValue = strval($arKeys[0]); // identries
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->id->CurrentValue = strval($arKeys[1]); // id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
