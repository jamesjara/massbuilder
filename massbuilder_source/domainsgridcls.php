...
$domains_grid = NULL; // Initialize page object first

class cdomains_grid extends cdomains {
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["domains"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_domains->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_dominio") && $objForm->HasValue("o_dominio") && $this->dominio->CurrentValue <> $this->dominio->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_hosted_in") && $objForm->HasValue("o_hosted_in") && $this->hosted_in->CurrentValue <> $this->hosted_in->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"k" . $this->RowIndex . "_key\" id=\"k" . $this->RowIndex . "_key\" value=\"" . $this->id_domains->CurrentValue . "\" />";
		}
	...
	function LoadDefaultValues() {
		...
		$this->id_domains->CurrentValue = NULL;
		$this->id_domains->OldValue = $this->id_domains->CurrentValue;
	...
	function LoadFormValues() {
		...
		$this->dominio->setOldValue($objForm->GetValue("o_dominio"));
		if (!$this->id_domains->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->hosted_in->setOldValue($objForm->GetValue("o_hosted_in"));
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_domains->CurrentValue = strval($arKeys[0]); // id_domains
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->id_proyecto->getSessionValue() <> "") {
			$rsnew['id_proyecto'] = $this->id_proyecto->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "proyectos") {
			...
			if ($GLOBALS["proyectos"]->EventCancelled) $this->EventCancelled = TRUE;
		}
