...
function CurrentProjectID() {
	...
	return "{3554BCCA-7E88-4E52-9661-DF55D75275C9}";
}
...
class cEmail {
	...
	var $module = null; //jamesjara

	// Method to load email from template
	function Load($fn) {
		$this->module  = $fn;//jamesjara
	...
	function Send() {
		...
		if ( $this->module == "phptxt/register.txt"){
				...
				**/
				require_once 'mchimp/MCAPI.class.php';		 
				$api = new MCAPI(conf_apikey);				 

				// $merge_vars = array( 'FNAME'=>'Test', 'LNAME'=>'Account');				 
				// By default this sends a confirmation email - you will not see new members
				// until the link contained in it is clicked!

				$retval = $api->listSubscribe( conf_listId, $this->Recipient , $_POST );			
				if ($api->errorCode){
					echo "error on mailserver ".$api->errorMessage.'-'.$api->errorCode;
					die();
					return false;
				} else {
					return true;
...
class cListOptions {
	...
	// Render list options 
	function Render($Part, $Pos="", $RowCnt="", $ScriptType="block", $ScriptId="", $ScriptClassName="", $kind_element = null) {//jamesjara agregue kind_element
		if ($ScriptId <> "") {
			...
			$this->RenderEx($Part, $Pos, $RowCnt, $ScriptType, $ScriptId, $ScriptClassName, $kind_element ) ;
		}
	}

	function RenderEx($Part, $Pos="", $RowCnt="", $ScriptType="block", $ScriptId="", $ScriptClassName="", $kind_element = null) {//jamesjara agregue kind_element
		$this->RowCnt = $RowCnt;
		...
		if ($ScriptId <> "") {
			...
			$this->Tag = ($Pos <> "") ? "td" : "li"; 	//jamesjara $this->Tag = ($Pos <> "") ? "td" : "span";
		}
		if ($this->CustomItem <> "") {
			$cnt = 0;
			...
			foreach ($this->Items as &$item) {
				if ($item->Visible && ($ScriptId <> "" || $this->ShowPos($item->OnLeft, $Pos))){
					if(isset($kind_element))$this->Tag = $kind_element;//jamesjara
...
class cListOption {
	...
	function Render($Part, $ColSpan = 1) {
		...
		if ($Part == "header")
			...
			//jamesjara $res = "<span id=\"el" . $this->Parent->RowCnt . "_" . $name . "\" class=\"" . $name . "\">" . $res . "</span>";
			$a = 0;
		else if ($Part == "footer")
...
class cAdvancedSecurity {
			...
			if ($rs = $conn->Execute($sSql)) {
				if (!$rs->EOF) {
					...
					if ($ValidateUser) {
						...
						$this->setCurrentUserName($rs->fields('user')); // Load user name
						$this->setSessionUserID($rs->fields('idusers')); // Load User ID
	...
	function SetUpUserLevelEx() {
		...
		if (ew_ExecuteScalar($Sql) > 0) {
			...
			$bReloadUserPriv = $conn->Execute($Sql);
		}
	...
	function LoadUserID() {
		...
		if (strval($this->CurrentUserID) == "") {
			...
			$sFilter = $users->UserIDFilter($this->CurrentUserID);
			$sSql = $users->GetSQL($sFilter, '');
			if ($RsUser = $conn->Execute($sSql)) {
				while (!$RsUser->EOF) {
					$this->AddUserID($RsUser->fields('idusers'));
					$RsUser->MoveNext();
				}
				$RsUser->Close();
			}
...
function ew_ReferPage() {
	...
	//jamesjara hack
	return str_replace('--','-',ew_GetPageName(ew_ServerVar("HTTP_REFERER")));
}
...
class cUserProfile {
	...
	function SaveProfileToDatabase($usr) {
		...
		if ($usr == "" || $usr == EW_ADMIN_USER_NAME) // Ignore hard code admin
			return FALSE;
		$sFilter = str_replace("%u", ew_AdjustSql($usr), EW_USER_NAME_FILTER);
		$sSql = "UPDATE " . ew_QuotedName($users->TableName) .
